...
| 2026-06-11 | Diagnostic v2 fixes (from Cowork report): content-block model fallback ladder (re-roll → Kling → element-free last resort) in generate-video; rights-verification job status parks as pending_external instead of failing; Play All fixed (persistent <video>, no remount, ended/src-swap pauses don't kill the chain); single-word element names match case-sensitively ("Ash" the character ≠ "ash" the debris); element rank+cap at 4 placeholders per generation with overflow demoted to continuity text; per-intent REST model env routing; assembly coverage validation (<30% blocks unless force, <70% warns; orchestrator forces); CLAUDE.md Gemini model corrected to gemini-3.1-flash-image-preview |
| 2026-06-11 | Diagnostic v2 large items: CROSS-PANEL CONTINUITY — first-frame generation now receives the previous panel's approved frame as a multimodal continuity reference (wardrobe state, lighting, set dressing, carried props persist shot-to-shot; SVG/missing frames skip silently). VIDEO STITCHING — ffmpeg-static devDependency (no Homebrew needed), stitch-film.mjs resolves PATH→static binary and falls back to re-encode concat on mixed codecs. Apex Hunter film stitched: single 2:16 MP4 (47MB) uploaded + stamped, Screening Room serves one file with download |
